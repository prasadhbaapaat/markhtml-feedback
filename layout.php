...
<head>
    ...
    <style><?= app_theme_style($config['theme'], $docTheme ?? []) ?></style>
    <!-- Google Fonts -->
    ...
    <div class="container-fluid">
        <a class="navbar-brand fw-bold d-flex align-items-center" href="?">
            <?php if (!empty($config['logo_path'])): ?>
                <img src="<?= h(app_asset($config['logo_path'])) ?>" alt="<?= h($config['logo_alt']) ?>" height="30" class="me-2">
            <?php else: ?>
                <?= h($config['brand_name']) ?>
            <?php endif; ?>
        </a>
        <div class="sidebar-title px-3 pt-2 mb-3">
                    <div class="small text-white">Document Heading: <?= h($documentTitle) ?>
                    </div>
                </div>
        <div class="d-flex align-items-center">
            <span class="text-white me-3 d-none d-md-inline">Welcome, <?= h($_SESSION['user_name'] ?? 'User') ?></span>
    ...
    <div class="row flex-grow-1">
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar collapse shadow-sm">
            <div class="position-sticky pt-3">
                ...
                <div class="px-3 mb-3">
                    <label class="form-label small text-uppercase fw-bold mb-1 px-2 py-1 rounded text-white w-100" style="background-color: var(--brand-primary);">Select Document</label>
                    <select class="form-select form-select-sm document-switcher" onchange="window.location.href='?doc=' + this.value">
                        <?php foreach ($availableDocuments as $docKey => $docConfig): ?>
                            <option value="<?= h($docKey) ?>" <?= $docKey === $documentId ? 'selected' : '' ?>><?= h($docConfig['title']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <?php endif; ?>
                ...
                <ul class="nav flex-column mb-auto">
                    <?php foreach ($navigation as $slug => $navItem): ?>
                        <li class="nav-item">
                            <a class="nav-link d-flex align-items-start gap-2 <?= $slug === $currentPageSlug ? 'active' : '' ?>" href="<?= h(app_page_url($slug, $documentId)) ?>">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            <?php if (!empty($pageError)): ?>
                <div class="alert alert-danger shadow-sm border-0 mt-4">
                    ...
                    <p>The document you are trying to access cannot be loaded. It may have been deleted or the configuration is incorrect.</p>
                    <hr>
                ...
                <div class="d-flex justify-content-between gap-3 mb-4">
                    <div>
                        <?php if (!empty($previousNext['previous'])): ?>
                            <a class="btn btn-secondary" href="<?= h(app_page_url($previousNext['previous']['slug'], $documentId)) ?>">&larr; <?= h($previousNext['previous']['title']) ?></a>
                        <?php endif; ?>
                    </div>
                    <div>
                        <?php if (!empty($previousNext['next'])): ?>
                            <a class="btn btn-secondary" href="<?= h(app_page_url($previousNext['next']['slug'], $documentId)) ?>"><?= h($previousNext['next']['title']) ?> &rarr;</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
                ...
                <div class="card-body">
                    ...
                    <div id="comments-list" class="mb-4">
                        <?php if (empty($comments)): ?>
                            <p class="text-muted" id="no-comments-msg">No comments yet. Be the first to suggest changes!</p>
                        <?php else: ?>
                            <?php foreach ($comments as $c): ?>
                                <div class="comment-thread mb-4">
                                    <div class="comment-item p-3 bg-white rounded shadow-sm border-0" data-comment-id="<?= $c['id'] ?>">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                <h6 class="mb-1 fw-bold">
                                                    <?= h($c['name']) ?>
                                                    <small class="text-muted fw-normal ms-2"><?= h($c['created_at']) ?></small>
                                                    <?php if (!empty($c['feedback_type'])): ?>
                                                        <span class="badge text-bg-primary ms-2"><?= h($c['feedback_type']) ?></span>
                                                    <?php endif; ?>
                                            ...
                                            <button class="btn btn-sm btn-outline-secondary reply-btn" data-parent-id="<?= $c['id'] ?>">Reply</button>
                                        </div>
                                        <p class="mb-0 mt-2 text-break" style="white-space: pre-line;"><?= h($c['comment']) ?></p>
                                    </div>
                                        ...
                                        <?php if (!empty($c['replies'])): ?>
                                            <?php foreach ($c['replies'] as $reply): ?>
                                                <div class="comment-item mb-3 p-3 bg-light rounded shadow-sm border-0">
                                                    <h6 class="mb-1 fw-bold">
                                                        <?= h($reply['name']) ?>
                                                        <small class="text-muted fw-normal ms-2"><?= h($reply['created_at']) ?></small>
                                                    </h6>
                                                    <p class="mb-0 text-break" style="white-space: pre-line;"><?= h($reply['comment']) ?></p>
                                                </div>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </div>
                                            ...
                                            <div class="row g-2 mb-2">
                                                ...
                                                <?php else: ?>
                                                    ...
                                                    <?php if (!empty($config['comments']['collect_email'])): ?>
                                                        <div class="col-md-6">
                                                            <input type="email" class="form-control form-control-sm" name="email" placeholder="Email (optional)">
                                                        </div>
                                                    <?php endif; ?>
                                                <?php endif; ?>
                                            </div>
                                            <div class="mb-2">
                                                <textarea class="form-control form-control-sm" name="comment" rows="2" required placeholder="Write a reply..."></textarea>
                                            </div>
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="replyFormMessage small"></div>
                                                <div>
                                                    <button type="button" class="btn btn-sm btn-light cancel-reply-btn">Cancel</button>
                        ...
                        <form id="commentForm">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" required placeholder="Please add name">
                                </div>
                                <?php if (!empty($config['comments']['collect_email'])): ?>
                                    <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="comment" name="comment" rows="4" placeholder="Enter your suggestions here..."></textarea>
                            </div>
